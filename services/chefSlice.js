...
export const chefLoginAsync = createAsyncThunk(
  ...
  async ({ kitchenId, password }, { rejectWithValue }) => {
    try {
      ...
      return res.data?.data;   // { chefId, name, kitchenId, restaurantId, accessToken }
...
const chefSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // ── Auth ────────────────────────────────────────────────
      .addCase(chefLoginAsync.pending,   (s) => { s.status.auth = "loading"; s.error = null; })
      ...
      .addCase(chefLogoutAsync.fulfilled, (s) => { Object.assign(s, initialState); })

      // ── Orders ──────────────────────────────────────────────
      .addCase(fetchKitchenOrdersAsync.pending,   (s) => { s.status.fetch = "loading"; })
      .addCase(fetchKitchenOrdersAsync.fulfilled,  (s, { payload }) => {
        s.status.fetch = "succeeded"; s.orders = payload;
      })
      .addCase(fetchKitchenOrdersAsync.rejected,   (s, { payload }) => {
        s.status.fetch = "failed"; s.error = payload;
      })

      .addCase(acceptOrderAsync.fulfilled,    (s, { payload }) => { s.orders = mergeOrder(s.orders, payload); })
      .addCase(rejectOrderAsync.fulfilled,    (s, { payload }) => { s.orders = mergeOrder(s.orders, payload); })
      .addCase(updatePrepTimeAsync.fulfilled, (s, { payload }) => { s.orders = mergeOrder(s.orders, payload); })
      .addCase(markOrderReadyAsync.fulfilled, (s, { payload }) => { s.orders = mergeOrder(s.orders, payload); })

      .addMatcher(
        (action) => action.type.startsWith("chef/") && action.type.endsWith("/rejected"),
...
export const selectPendingOrders   = (s) => s.chef.orders.filter((o) => o.status === "pending");
export const selectAcceptedOrders  = (s) => s.chef.orders.filter((o) => o.status === "accepted");
export const selectReadyOrders     = (s) => s.chef.orders.filter((o) => o.status === "ready");
